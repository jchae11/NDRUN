...
/*---------------------------------------------------------------------------*/
		...
		// Values for applying artifaccts texture
		private Texture2D p_artifacts = null;
		private float p_pivot = 0.4f;
		private float p_strength = 1f;
		private float p_neutral = 0.1f;
...
/*---------------------------------------------------------------------------*/
	...
	public void drawAdvancedSettingsGUI( ){
		...
		EditorGUILayout.BeginVertical();
			...
			// --------------------------------------------------------------------
			// ADD ARTIFACTS ON TERRAIN
			
			GUI.skin = p_skin;
			GUILayout.Box("Artifacts");
			GUI.skin = null;
			
			EditorGUILayout.Separator();
			
			p_artifacts = EditorGUILayout.ObjectField("Artifacts texture", p_artifacts, typeof(Texture2D),false) as Texture2D;
			
			EditorGUILayout.HelpBox("Don't forget to check [Read/Write enabled] for the texture !",MessageType.Info);
						
			EditorGUILayout.BeginHorizontal();
				EditorGUILayout.PrefixLabel("Apply at normalized height");
				p_pivot = EditorGUILayout.Slider(p_pivot, 0f, 1f);			
			EditorGUILayout.EndHorizontal();
			
			EditorGUILayout.BeginHorizontal();						
				EditorGUILayout.PrefixLabel("Neutral zone around (%)");
				p_neutral = EditorGUILayout.Slider(p_neutral, 0.01f, 1f);
			EditorGUILayout.EndHorizontal();
			
			EditorGUILayout.BeginHorizontal();						
				EditorGUILayout.PrefixLabel("Strength multiplicator");
				p_strength = EditorGUILayout.Slider(p_strength, 0.01f, 10f);
			EditorGUILayout.EndHorizontal();			
			
			EditorGUILayout.Separator();
			
			if( GUILayout.Button("Apply Artifacts\n[RE-APPLY TEXTURES]") ){
				...
				// Reset terrain
				toolkit.applyArtifacts(p_artifacts,p_pivot,p_neutral,p_strength);
